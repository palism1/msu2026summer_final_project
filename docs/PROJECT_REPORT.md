...
This report presents a controlled study of parameter-efficient adaptation of segmentation
foundation models for colonoscopy polyp segmentation. The study asks one question: can the
Segment Anything Model (SAM), adapted with Low-Rank Adaptation (LoRA) while its backbone stays
frozen, match a specialist U-Net on the standard PraNet benchmark while it degrades less on
datasets neither model trained on?
...
The seen-to-unseen drop does.

The study trained six model variants under one shared protocol, three random seeds each, on
one NVIDIA A100 GPU, and evaluated four untrained oracle-box baselines, for 22 runs in total.
The key findings:
...
3. **A measured diagnosis of MedSAM's deficit.** MedSAM's apparent collapse (0.661
   ...
   preprocessing bug in our pipeline cost 0.069 Dice; with backbone and preprocessing
   matched, the MedSAM checkpoint still trails generic SAM by 0.036, comparable in size
   to the 0.045 cost of the smaller backbone. The initial reading, that medical
   pretraining is a net drag, was retracted after the controlled runs.
...
two controlled pairings that separate the pretraining source from the backbone size plus a
measured preprocessing effect, a documented
root-cause investigation of a preprocessing confound, and a reproducible config-driven
pipeline with all results consolidated in one summary.
...
CNNs lose substantial accuracy on the unseen splits, which made cross-dataset generalization a
first-class metric for this task. Polyp-PVT [3] pushed specialist accuracy further with a
...
**The unanswered question.** The surveyed work reports benchmark accuracy. None of it answers
whether LoRA adaptation of a foundation model buys *generalization*, measured on the same
seen/unseen protocol the specialists were built on, under one training protocol, at matched
cost accounting. Two published claims also sit in tension: MedSAM reports beating specialists
on endoscopy, while the PraNet protocol is exactly the setting where such a claim can be
tested without a prompt oracle. This study fills both holes.
...
to determine whether parameter-efficient adaptation captures the foundation model's
generalization at specialist-level benchmark cost, and to attribute any differences to their
actual causes rather than to confounds.
...
MedSAM an advantage over generic SAM at equal backbone size. The second hypothesis was not
supported; Section 6 reports the measured comparisons, including the part of the deficit
our own pipeline caused.
...
from-scratch training. The MedSAM analysis additionally shows how a preprocessing contract
violation can masquerade as a scientific finding, which is a methods lesson beyond this task.
...
from pretraining source. Two caveats bound these pairings. The LoRA parameter count scales
with the backbone (830K on ViT-H against 322K on ViT-B), so the size comparison moves adapter
capacity together with backbone capacity. And MedSAM's starting weights bundle two changes its
authors made together, medical training data and a box prompt supplied on every training
example, so the pretraining pair measures the checkpoint as delivered, not the medical data
alone.
...
ground-truth box reveals the polyp's location, so these numbers form an upper bound, kept in
a separate tier and never ranked against the trained models. The box protocol is standard in
the medical-SAM literature and was confirmed with the professor.

### 4.6 The MedSAM three-arm experiment

A review on 2026-07-28 found that the pipeline fed every model ImageNet-standardized inputs.
That is correct for SAM by construction and wrong for MedSAM, whose frozen encoder was
fine-tuned on per-image min-max [0,1] inputs. A direct re-run under min-max would have changed
two things at once, because min-max normalization is exactly invariant to brightness and
contrast jitter, so correcting the normalization also weakens the effective augmentation.
The design therefore uses three arms:
...
Two implementation decisions deserve note. First, the training protocol (normalization and
photometric augmentation) is bound to the model key in `src/config.MODEL_SPECS`, not settable
in YAML. This makes a protocol mismatch a code change with review, not a silent config drift.
Second, checkpoints follow one path contract (`checkpoints/<model>/seed<seed>/best.pt`), which
lets the aggregator and the benchmark notebook find every run mechanically.
...
last on every split (0.661 unseen, 0.500 on ETIS-LaribPolypDB), and the first write-up
attributed this to medical pretraining. A root-cause investigation
(`docs/MEDSAM_INVESTIGATION.md`) ranked four hypotheses and found that the pipeline's single
global ImageNet normalization violated MedSAM's input contract: its encoder was fit on [0,1]
inputs and received inputs in roughly [−2.1, +2.6]. The fix bound normalization to the model
key. The confound analysis then showed the naive fix would itself be confounded (min-max
absorbs brightness/contrast jitter), which produced the three-arm design in Section 4.6.
The published `medsam` arm was preserved and never re-run; the corrected arms got separate
model keys, checkpoints, and results paths.

**Prompt protocol for the untrained tier.** A zero-shot SAM needs a prompt. A ground-truth
box is an oracle: it hands the model the location. The resolution was structural: keep the
oracle tier in a separate table, flag the negative seen-to-unseen gap as the sign that these
numbers track split difficulty rather than learning, and get the professor's sign-off on the
box protocol before the final runs.
...
Early stopping kept several 50-epoch budgets to 28-49 actual epochs.

**Testing without a GPU.** Prompt-derivation math and the config/aggregation layer are
torch-free and covered by 24 unit tests (`pytest tests/`), so the logic that defines the
experiment could be verified locally before spending GPU time.
...
U-Net. Which matters depends on whether the target is the benchmark or the next clinic. The
whole study (18 trained runs) took about 750 GPU-minutes, or 12.5 A100-hours; at Colab rates
that is roughly 15 dollars of compute (measured GPU-hours, estimated rates).
...
Generalization, not learning ability, is where they separate.
...
Of MedSAM's original 0.100 unseen deficit against SAM-ViT-B, 0.069 was our preprocessing bug
...
the three-arm design. The earlier claim that medical pretraining is a net drag and accounts for
most of the deficit was retracted on this evidence.
...
Two reads. First, the normalization bug replicates with zero training: the same MedSAM
...
unseen data than seen data. Prompt dependence, quantified: the untrained box-prompted model
...
gap metric makes the trade legible: 3% of the trainable parameters buys half the
generalization drop.

**MedSAM is specialized, not weak.** The study's most instructive finding is negative
in form. MedSAM's published claim (specialist-level endoscopy accuracy) and our measured
...
the prompt. Practitioners choosing a model should ask which contract their deployment
honors: if no localizer supplies a box at inference, prompt-free adaptation of generic
SAM beats medical-pretrained MedSAM at the same size.

**Methods lesson: preprocessing contracts are results.** A one-line normalization
difference produced a 0.069 Dice artifact that initially read as a scientific finding
about medical pretraining. The correction required a controlled experiment because the
naive fix was itself confounded through the interaction between min-max normalization
and photometric augmentation. Binding preprocessing to the model key in code, with the
published arm preserved and the corrected arms given separate keys, is the reusable
pattern.
...
measured causes: a 0.069 preprocessing artifact in our own pipeline, a 0.036 residual for
...
**Limitations.** One benchmark family: all five splits come from PraNet's datasets, so the
numbers do not speak to colonoscopy data under other equipment or protocols. Three seeds
show the spread is real but the sample is small. The oracle tier has one run per baseline
and remains an upper bound, not a peer. Training cost is asymmetric in a way that matters
to some deployments (U-Net: 10 min, 98 MB; SAM-H: 120 min, 2.5 GB).

**Future work.** Evaluate on a benchmark family outside PraNet's five datasets; probe H3
(color sensitivity) with the grayscale-replication experiment already designed; test
larger LoRA ranks and target modules; pair MedSAM with a learned box proposer to test
whether its oracle ceiling is reachable without ground truth.
