...
ABSTRACT = r"""Specialist polyp segmentation networks fit their training distribution tightly and lose accuracy on data from other clinics. This study asks whether the Segment Anything Model (SAM), adapted with Low-Rank Adaptation (LoRA) while its backbone stays frozen, matches a specialist U-Net on the PraNet benchmark and loses less accuracy on the three datasets neither model trained on. Six model variants were trained under one protocol, three seeds each, on one NVIDIA A100, and four untrained oracle-box baselines were evaluated, for 22 runs in total. SAM-ViT-H + LoRA scores highest on the unseen splits (0.806 mean Dice against U-Net's 0.755) with a seen-to-unseen drop about half of U-Net's (0.082 against 0.145), while it trains 830,177 parameters, about 3\% of the 24.4M that U-Net trains end-to-end. The specialist keeps the edge on seen data (0.900) and trains twelve times faster. My first reading was that medical pretraining is a net drag. A controlled three-arm experiment retracted that: it splits MedSAM's 0.100 unseen deficit against SAM-ViT-B into a 0.069 preprocessing artifact in my own pipeline and a 0.036 residual for the checkpoint at matched backbone size. With a ground-truth box and its native min-max inputs, untrained MedSAM reaches 0.925 unseen Dice, the highest number in the oracle tier, and the size of that lift (+0.200 against +0.099 for generic SAM) quantifies its prompt dependence. All numbers rest on one benchmark family and three seeds, with no significance test."""
